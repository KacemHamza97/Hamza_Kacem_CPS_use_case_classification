...
def main(config):
    ...
    data_loader = DataLoader(config['data']['training_data_path'], config['data']['response_data_path'])
    ...
    for model_name, model_config in config['models'].items():
        ...
        n_iter = model_config.get('n_iter', 2)  # Default to 10 iterations if not specified
        logger.info(f"Tuning hyperparameters for model {model_name} with param_dist: {param_dist}")

        pipeline = create_pipeline(model_name, {})
        random_search = RandomizedSearchCV(pipeline, param_distributions=param_dist, n_iter=n_iter, cv=2, scoring=scorer, random_state=42)
